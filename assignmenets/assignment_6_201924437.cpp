...
MySet MySet::operator+(const MySet&); // make the union of two sets and return the result

MySet MySet::operator*(const MySet&); // make the intersection of two sets and return the result

MySet MySet::operator-(const MySet&); // make the difference A-B and return the result

Int MySet::operator[](float) //return the rank (starting from 1) of given key value in [ ]. i.e. A[25.3] returns the rank of value 25.3. If no element exists, return -1.

ostream& operator<<(ostream&, MySet) // output stream in text such as {10.1, 20.1, 30.1} always sorted in ascending order and the end of line at the end
